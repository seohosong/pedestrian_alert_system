...
def detect_and_save(frame_buffers):
    for frame in frame_buffers:
        for det in frame:
            for obj in det:
                center = ((obj[0] + obj[2]) / 2, (obj[3] + obj[1]) / 2)
                cls = int(obj[5].detach().cpu().item())
                area = (obj[2] - obj[0]) * (obj[3] - obj[1])
    return center[0] >= width_range[0] and center[0] <= width_range[1] and center[1] >= height_range[0] and center[1] <= height_range[1] and area >= area_thres


def detect_alarm(frame_buffers, names, im0s, area_thres=8000, show=False):
    img_size = im0s.shape
    width_range = (img_size[1] / 3, img_size[1] / 3 * 2) #Alert area
    height_range = (img_size[0] / 3, img_size[0] / 3 * 3) #Alert area

    names_dict = dict((name, 0) for name in names)
    names_areas = dict((name, []) for name in names)

    for frame in frame_buffers:
        for det in frame:
    #pool = Pool()
    #results = pool.map(detect_and_save, det)
            for obj in det:
                center = ((obj[0] + obj[2]) / 2, (obj[3] + obj[1]) / 2)
                cls = int(obj[5].detach().cpu().item())
                area = (obj[2] - obj[0]) * (obj[3] - obj[1])
                if center[0] >= width_range[0] and center[0] <= width_range[1] and center[1] >= height_range[0] and center[1] <= height_range[1] and area >= area_thres:
                    names_dict[names[cls]] += 1
                    #names_areas[names[cls]].append(area.detach().cpu().item())

    det = frame_buffers[-1][0]
    alarm = False
    for name in names_dict:
        if names_dict[name] >= 20:
            alarm = True
            ...
            for obj in det:
                ...
                if names[int(obj[5].detach().cpu().item())] == name:
                    center = ((obj[0] + obj[2]) / 2, (obj[3] + obj[1]) / 2)
                    area = (obj[2] - obj[0]) * (obj[3] - obj[1])
                    if center[0] >= width_range[0] and center[0] <= width_range[1] and center[1] >= height_range[0] and center[1] <= height_range[1] and area >= area_thres:
                        obj[6] = 1
                        names_areas[name].append(area.item())
    name_max = []
    for name in names_areas:
        if len(names_areas[name]) > 0:
            name_max.append(np.array(names_areas[name]).max())
        else:
            name_max.append(0)
    name_max = names[torch.max(torch.tensor(name_max), dim=0)[1].item()]

    return det, alarm, name_max
